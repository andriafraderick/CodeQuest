...
class Filehand extends StatefulWidget {
  const Filehand({super.key});

  @override
  _FilehandState createState() => _FilehandState();
}

class _FilehandState extends State<Filehand> {
  ...
  Future<void> getLessonProgress() async {
    ...
    if (doc.exists) {
      ...
      setState(() {
        isLessonComplete = data['intlessons']?['File handling'] ?? false;
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Python Filehand & Runner'),
        centerTitle: true,
      ),
      body: Container(
  ...
  List<Widget> buildLessonCards() {
    return [
      buildLessonCard(
        title: '1. Print Statement',
        code: 'print("Hello, World!")',
        description: 'Use `print()` to display text on the screen.',
        lessonKey: 'Filehandling',
      ),
    ];
  }
